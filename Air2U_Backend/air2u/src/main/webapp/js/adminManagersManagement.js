...
$(document).ready(function() {
	...
	$('.Admin-ManagerUser').addClass("active");
	
	loadPageContentData();
	getRolesEdit();
	
	//$('#resetPwdHref').unbind('click').bind('click',sendEmailForPWd());
	...
	pullDataFromCloudFunction("Users/"+userID,"READ","",expand,showUserNameToInput,filter);
	
	if($("#userRoleValue").val().indexOf("Root")!="-1"){
		
	}else{
		$(".btn-add-manager").attr("disabled","disabled");
	}
	...
	$(".select-opr").unbind('click').bind('click', function(){
		...
		var roles = {"AppRoles":{"$all":["ce7416c0-1b69-11e7-9f4f-41e0aef636ba"] } };
		filters.push(roles);
		...
		currentPage =0;
		pullDataFromCloudFunction("Users","READ","",expand,listAllManagersCallback,filter2,0, 10);
...
function showUserNameToInput(data){
	var userDetails = data.Result;
	
	$("input[name='hid_userName']").val(userDetails.Email);
...
function loadPageContentData(){
	...
	var filter ={"AppRoles":{"$all":["ce7416c0-1b69-11e7-9f4f-41e0aef636ba"] } };
	
	pullDataFromCloudFunction("Users","READ","",expand,listAllManagersCallback,filter,0, 10);
}
...
function previousAdd(){
	...
	if(currentPage < 0){
		...
		pullDataFromCloudFunction("Users","READ","",expand,listAllManagersCallback,filter2, currentPage, 10);
		
		//pullDataFromCloudFunction("Users","READ","",expand,listAllManagersCallback,filter,currentPage, 10);
	}
};
...
function writeDataToRoleSelector(data){
	...
	for ( var index in roles) {
		...
		if(roleRow.RoleName.indexOf("Manager")!="-1"){
...
function listAllManagersCallback(data){
	allUserNames = "";
	allEmailNames = "";
	...
	for ( var index in allAdminUsers) {
		...
		for (var int = 0; int < userRoleArray.length; int++) {
			...
			if (currentRoleName.indexOf("Manager")!="-1") {
				...
				strBuffer.append("<tr rowID='"+adminUserRow.Id+"'>");
					...
					if($("#userRoleValue").val().indexOf("Root")!="-1"){
						...
						if($("input[name='hid_userName']").val() == adminUserRow.Email){
							strBuffer.append("<td><i class='fa fa-edit fa-edit-style cursorPointer' style='color: green;' onclick='updateUser(this)'></i>   " +
									"<i class='glyphicon glyphicon-remove cursorPointer deletemanager' style='color: gray;' " +
									"data-toggle='popover' data-placement='bottom' data-content='You donot have permission.'></i></td>");
							
						}else{
							strBuffer.append("<td><i class='fa fa-edit fa-edit-style cursorPointer editmanager' style='color: gray;' " +
									"data-toggle='popover' data-placement='bottom' data-content='You donot have permission.'></i>" +
									"<i class='glyphicon glyphicon-remove cursorPointer deletemanager' style='color: gray;' " +
									"data-toggle='popover' data-placement='bottom' data-content='You donot have permission.'></i></td>");
						}
	...
	$(".editmanager").popover({html:true,trigger:'click'}).each(function(){
		$(this).find('.popover-content').html('<button type="button" class="btn btn-default">Sorry,you donot have permission.</button>');
	});
	
	$(".deletemanager").popover({html:true,trigger:'click'}).each(function(){
		$(this).find('.popover-content').html('<button type="button" class="btn btn-default">Sorry,you donot have permission.</button>');
	});
}
...
function updateUser(obj){
	...
	$("#userRoleSelectEdit").val("Manager");
...
function saveUserEdit(){
	...
	var userObj = { "Username" : $('#userNameEdit').val().toLowerCase(),
